...
Restoration branch: six editable layer kinds are restored on the sole recovered V0.6.9 base, including embedded material pixels in the existing version1 `.drawstudio` format. See `../recovery/V067_CHECKPOINT.md` for the current verification/release status. Freehand balloon contours and arbitrary polygon frames remain deferred. V0.6.8 is not included in this checkpoint.
